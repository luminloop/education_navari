...
def get_paystack_settings():
	...
	paystack_key = getattr(settings, 'paystack_key', None)
	paystack_secret = getattr(settings, 'paystack_secret', None)
	if not paystack_key and not paystack_secret:
		frappe.throw(
			_(
	...
	return {
		...
		"secret_key": paystack_secret.get_password() if hasattr(paystack_secret, 'get_password') else paystack_secret
	}
...
def get_payment_options(doctype, docname, phone, currency=None):
	...
	if paystack_response.get("status") and paystack_response.get("data"):
		...
		options = {
			...
			"amount": int(float(details.outstanding_amount) * 100),  # Amount in kobo/cents
...
def create_paystack_payment_record(args, status):
	...
	payment_record.amount = args.get("amount", 0) / 100 if args.get("amount") else 0  # Convert from kobo
	if status == "Success":
		...
		payment_record.address_line_1 = args.get("metadata", {}).get("address_line_1", "")
		payment_record.currency = args.get("currency", "")
		payment_record.address_line_2 = args.get("metadata", {}).get("address_line_2", "")
	...
	if status == "Failed":
		payment_record.failure_description = args.get("gateway_response", {}).get("message", "")
...
def handle_payment_success(response, against_invoice, billing_details):
	...
	try:
		...
		if not verification_data.get("status") or verification_data["data"]["status"] != "success":
